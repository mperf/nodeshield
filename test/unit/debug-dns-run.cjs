...
let createDnsShimCodeCjs;
// load ESM module dynamically
async function loadHelpers() {
  const mod = await import('../../src/box/internals/dns.js');
  createDnsShimCodeCjs = mod.createDnsShimCodeCjs;
}
...
async function main(){
  ...
  let patched = src.replace(/const hostModule = require\("node:dns"\);/, fakeHost);
  // inject a debug log inside the wrapper to observe runtime context
  patched = patched.replace("return function wrapped(...args) {", "return function wrapped(...args) { console.log('WRAP_getContext=', JSON.stringify(getContext()));");
